...
function reportGameOver(chess, plLast, movesAmount, avgTime) {
    ...
    if (chess.in_checkmate()) {
        if (plLast) {
            endgame_status = "You won";
        } else {
            endgame_status = "Random won !!!";
        }
    } else if (checkMove.in_stalemate()) {
        endgame_status = "Stalemate";
    } else {
        endgame_status = "No clue what happened here.";
    }

    const result = `Amount of moves: ${movesAmount}; Average move time: ${timeFmt(
        avgTime
    )}; End game status: ${endgame_status} \n`;

    fs.appendFileSync("results.txt", result);
    console.log("Saving results output to 'results.txt'");
...
function runTest(x, totalTime, chess) {
    if (chess.game_over()) {
        reportGameOver(chess, false, x, totalTime / x);
        return;
    }
    ...
    if (x < 1000) {
        ...
        run(chess.fen(), fens)
            .then(function ({ fen, time }) {
                ...
                console.log(
                    `🤖👌 PL OK (${timeFmt(time)}, avg: ${timeFmt(avgTime)})`
                );
                ...
                if (x > 5 && avgTime > 30 * 1000) {
                    ...
                    if (plChess.game_over()) {
                        reportGameOver(plChess, true, x + 1, avgTime);
                    } else {
